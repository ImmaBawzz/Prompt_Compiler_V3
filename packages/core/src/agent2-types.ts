...
export type BehavioralWeightDimension =
  | 'self-modification-threshold'
  | 'code-change-confidence'
  | 'test-first-priority'
  | 'task-selection-breadth'
  | 'git-push-eagerness'
  | 'learning-cycle-frequency'
  | 'shadow-eval-strictness';
...
export const DEFAULT_BEHAVIORAL_WEIGHTS: Record<BehavioralWeightDimension, number> = {
...
export function createDefaultBehavioralWeights(now = new Date().toISOString()): BehavioralWeight[] {
  return (Object.keys(DEFAULT_BEHAVIORAL_WEIGHTS) as BehavioralWeightDimension[]).map((dimension) => ({
    dimension,
    currentValue: DEFAULT_BEHAVIORAL_WEIGHTS[dimension],
    status: 'active',
    sampleCount: 0,
    updatedAt: now
  }));

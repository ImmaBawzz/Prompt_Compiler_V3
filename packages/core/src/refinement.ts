 ...
 */
export function deriveRefinementHints(bundle: CompilationBundle): RefinementHint[] {
 ...
 */
export function refinePromptBundle(
  brief: PromptBrief,
  profile: BrandProfile,
  context: RefinementContext
): CompilationBundle {
  let refinedBrief: PromptBrief = { ...brief };
  let refinedProfile: BrandProfile = { ...profile };
  ...
  const bundle = compilePromptBundle(refinedBrief, refinedProfile);
...
function applyHintToBrief(brief: PromptBrief, hint: RefinementHint): PromptBrief {
  switch (hint.type) {
    case 'add-constraint': {
    ...
    case 'add-target': {
      const targetValue = hint.value?.trim() as CompileTarget | undefined;
    ...
    case 'remove-target': {
      const targetValue = hint.target ?? (hint.value?.trim() as CompileTarget | undefined);
      if (targetValue) {
...
function applyHintToProfile(profile: BrandProfile, hint: RefinementHint): BrandProfile {
